                ...
                <tbody id="transactionsTableBody">
                    @forelse($expenses ?? [] as $expense)
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 text-sm text-gray-900">{{ $expense->category->particulars_category ?? '-' }}</td>
...
<script>
    ...
    openingBalanceForm.addEventListener('submit', function(e) {
        ...
        fetch('{{ route("accounting.update-opening-balance") }}', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('expenseForm').addEventListener('submit', function(e) {
        ...
        fetch('{{ route("accounting.store-expense") }}', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
    ...
    document.querySelectorAll('.deleteBtn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (confirm('Are you sure you want to delete this transaction?')) {
                fetch(`{{ route("accounting.delete-expense", ":id") }}`.replace(':id', expenseId), {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]')?.content || document.querySelector('input[name="_token"]')?.value
                    }
