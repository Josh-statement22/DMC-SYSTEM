...
<script>
	...
	async function fetchCashAdvanceRequests() {
		...
		console.log('Fetched cash advance requests:', cashAdvanceRequestsCache.length, 'records');
		console.log('First few records:', cashAdvanceRequestsCache.slice(0, 3));
	...
	function renderRecentDisbursements() {
		console.log('Rendering recent disbursements with', cashAdvanceRequestsCache.length, 'cached records');
		const allRequests = [...cashAdvanceRequestsCache]
		...
		const rows = allRequests.slice(0, 12).map(request => {
			...
			const processedBy = request.sent_by_name || request.approved_by_name || '-';
		...
		console.log('Rendering', allRequests.slice(0, 12).length, 'rows in table');
	...
	if (sendCashAdvanceForm) {
		sendCashAdvanceForm.addEventListener('submit', async function(event) {
			...
			console.log('Form submitted - Send Money');
			...
			console.log('Sending cash advance:', { requesterId, amount, purpose, releaseDate });

			const sendMoneyBtn = document.getElementById('sendMoneyBtn');
			const sendMoneyBtnText = document.getElementById('sendMoneyBtnText');
			const originalText = sendMoneyBtnText.textContent;
			
			try {
				// Show loading state
				sendMoneyBtn.disabled = true;
				sendMoneyBtnText.textContent = 'Sending...';

				console.log('Calling API:', CASH_ADVANCE_DIRECT_SEND_ROUTE);
				...
				console.log('API Response:', { ok: response.ok, status: response.status, payload });
				
				if (!response.ok) {
					sendMoneyBtnText.textContent = originalText;
					sendMoneyBtn.disabled = false;
					showAccountingToast(payload?.message || `Server error: ${response.status}`, 'error');
					return;
				}

				// Reset form and button immediately for better UX
				...
				console.log('Button reset and success shown - starting async refresh');
				
				// Refresh data in background asynchronously without blocking UI
				(async () => {
					try {
						console.log('Background: Fetching updated cash advance requests...');
						await fetchCashAdvanceRequests();
						console.log('Background: Rendering updated UI...');
						renderQueue();
						renderBudgetWindowAmounts();
						renderRecentDisbursements();
						feather.replace();
				...
				console.error('Send cash advance error:', error);
				sendMoneyBtnText.textContent = originalText;
				sendMoneyBtn.disabled = false;
	...
	async function initializeAccountingDashboard() {
		...
		renderQueue();
		renderRecentDisbursements();
		// TODO: Re-enable after fixing SSE stream performance
